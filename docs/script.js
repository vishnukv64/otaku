...
function detectOS() {
    ...
    if (platform.includes('mac')) {
        ...
        // Strategy: Use multiple detection methods for better accuracy
        let isARM = false;

        // Method 1: Check for ARM64 in platform (most reliable when available)
        if (platform.includes('arm')) {
            isARM = true;
        }

        // Method 2: Check WebGL renderer for Apple GPU indicators
        try {
            ...
            if (gl) {
                ...
                if (debugInfo) {
                    ...
                    // Apple Silicon uses Apple M1/M2/M3/M4 GPU
        ...
        // Method 3: Default to Apple Silicon for macOS 11+ (Big Sur and later)
        // Most Macs sold since 2020 are Apple Silicon
        if (!isARM) {
            const match = userAgent.match(/mac os x (\d+)[._](\d+)/);
            if (match) {
                const majorVersion = parseInt(match[1]);
                // macOS 11 (Big Sur) and later - more likely to be Apple Silicon
                // Default to ARM, user can select Intel from secondary links if needed
                if (majorVersion >= 11) {
                    isARM = true;
                }
            }
        ...
        return {
            os: 'mac',
            arch: isARM ? 'aarch64' : 'x64',
            displayName: isARM ? 'macOS Apple Silicon' : 'macOS Intel'
        };
    }
...
// ===== GitHub API =====
const GITHUB_REPO = 'vishnukv64/otaku';
const GITHUB_API_URL = `https://api.github.com/repos/${GITHUB_REPO}/releases/latest`;

// Asset patterns for matching
const assetPatterns = {
...
function matchAsset(os, arch, assets) {
    let pattern;

    switch (os) {
        case 'windows':
            // Prefer setup.exe over MSI
            pattern = assetPatterns.windowsSetup;
            let asset = assets.find(a => pattern.test(a.name));
            if (!asset) {
                pattern = assetPatterns.windowsMsi;
                asset = assets.find(a => pattern.test(a.name));
            }
            return asset;

        case 'mac':
            pattern = arch === 'aarch64' ? assetPatterns.macArm : assetPatterns.macIntel;
            return assets.find(a => pattern.test(a.name));

        case 'linux':
            // Prefer AppImage (universal)
            pattern = assetPatterns.linuxAppImage;
            let linuxAsset = assets.find(a => pattern.test(a.name));
            if (!linuxAsset) {
                pattern = assetPatterns.linuxDeb;
                linuxAsset = assets.find(a => pattern.test(a.name));
            }
            if (!linuxAsset) {
                pattern = assetPatterns.linuxRpm;
                linuxAsset = assets.find(a => pattern.test(a.name));
            }
            return linuxAsset;
...
function getAllDownloadOptions(assets) {
    return {
        windowsSetup: assets.find(a => assetPatterns.windowsSetup.test(a.name)),
        windowsMsi: assets.find(a => assetPatterns.windowsMsi.test(a.name)),
        macIntel: assets.find(a => assetPatterns.macIntel.test(a.name)),
...
function updateDownloadButton(buttonId, textId, asset, version, osDisplayName) {
    const button = document.getElementById(buttonId);
    const text = document.getElementById(textId);

    if (asset) {
        text.textContent = `Download for ${osDisplayName}`;
        button.disabled = false;
        button.onclick = () => window.location.href = asset.downloadUrl;
    } else {
        text.textContent = 'Download Not Available';
        button.disabled = true;
    }
}

// Create platform links
function createPlatformLinks(currentOS, currentArch, allOptions) {
    const platformsContainer = document.getElementById('hero-platforms');
    platformsContainer.innerHTML = '';

    const platforms = [];

    // Add other platforms
    if (currentOS !== 'windows' && allOptions.windowsSetup) {
        platforms.push({
            ...
            label: 'Windows',
            asset: allOptions.windowsSetup
        });
    }

    if (currentOS !== 'mac') {
        if (allOptions.macArm) {
            platforms.push({
                ...
                label: 'macOS',
                asset: allOptions.macArm
            });
        } else if (allOptions.macIntel) {
            platforms.push({
                icon: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>`,
                label: 'macOS',
                asset: allOptions.macIntel
            });
        }
    } else {
        // Show the other Mac architecture
        if (currentArch === 'aarch64' && allOptions.macIntel) {
            platforms.push({
                icon: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>`,
                label: 'macOS Intel',
                asset: allOptions.macIntel
            });
        } else if (currentArch === 'x64' && allOptions.macArm) {
            platforms.push({
                icon: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>`,
...
function setupCarousel() {
    ...
    function startAutoplay() {
        autoplayInterval = setInterval(nextSlide, 4500); // Change slide every 4.5 seconds
    }
    ...
    if (carousel) {
        carousel.closest('.carousel').addEventListener('mouseenter', stopAutoplay);
        carousel.closest('.carousel').addEventListener('mouseleave', startAutoplay);
    }
...
async function init() {
    // Detect OS
    const { os, arch, displayName } = detectOS();
    console.log('Detected OS:', displayName, '(', os, arch, ')');
    ...
    // Match primary asset
    const primaryAsset = matchAsset(os, arch, release.assets);
    ...
    // Create platform links
    createPlatformLinks(os, arch, allOptions);

    // Setup screenshot carousel
    setupCarousel();
}
